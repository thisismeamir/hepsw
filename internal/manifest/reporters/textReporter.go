...
func textHeaderGenerator(m *manifest.Manifest) string {
...
func textBasicInformationGenerator(m *manifest.Manifest) string {
	var sb strings.Builder
...
func textSourceInfoGenerator(m *manifest.Manifest) string {
	var sb strings.Builder
	// Source
...
func textMetadataGenerator(m *manifest.Manifest) string {
	var sb strings.Builder
	// Metadata
...
func textBuildSpecificationsGenerator(m *manifest.Manifest) string {
	var sb strings.Builder
...
func textDependencyGenerator(m *manifest.Manifest) string {
	var sb strings.Builder
	// Dependencies
...
func textEnvironmentGenerator(m *manifest.Manifest) string {
	var sb strings.Builder
...
func textRecipeSummaryGenerator(m *manifest.Manifest) string {
	var sb strings.Builder
	// Recipe Summary
...
func generateTextReport(m *manifest.Manifest) string {
	var sb strings.Builder

	header := textHeaderGenerator(m)
	sb.WriteString(header)

	basicInformation := textBasicInformationGenerator(m)
	sb.WriteString(basicInformation)

	sourceInformation := textSourceInfoGenerator(m)
	sb.WriteString(sourceInformation)

	metadataInformation := textMetadataGenerator(m)
	sb.WriteString(metadataInformation)

	buildSpecifications := textBuildSpecificationsGenerator(m)
	sb.WriteString(buildSpecifications)

	dependencyInformation := textDependencyGenerator(m)
	sb.WriteString(dependencyInformation)

	environmentInformation := textEnvironmentGenerator(m)
	sb.WriteString(environmentInformation)

	recipeSummary := textRecipeSummaryGenerator(m)
	sb.WriteString(recipeSummary)
...
func printRecipePhase(sb *strings.Builder, phaseName string, steps []manifest.RecipeStep) {
	if len(steps) == 0 {
		return
	}
